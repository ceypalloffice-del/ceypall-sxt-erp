...
/** Generic add form for categories without a structured entry (nails, labour, …). */
function AddMaterialCard({ categories, title }: { categories: string[]; title: string }) {
  return (
    <Card>
      <h2 className="text-sm font-semibold text-slate-700">{title}</h2>
      <form action={createCostItem} className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-4">
        <input name="name" placeholder="Item name" required className={`${inputCls} sm:col-span-2`} />
        ...
        addForms={{
          ...
          Others: (
            <AddMaterialCard
              ...
              categories={["chemical", "labour", "transport", "other", "timber"]}
            />
          ),
        }}
      />
